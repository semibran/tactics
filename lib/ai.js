...
export function analyze(map, faction) {
	...
	for (let unit of units) {
		...
		let targets = []
		if (unit.ai === "defend") {
			let range = Cell.neighborhood(unit.cell, Unit.rng(unit.type))
			...
			if (targets.length) {
				...
				if (targets.length) {
					let elevation = Map.tileAt(map, unit.cell)
					let paths = targets.map(target => pathfind(map, unit.cell, target.cell).slice(0, -1))
